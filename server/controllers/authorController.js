...
exports.getAuthor = async (req, res) => {
  ...
  Author.findById(address)
    .then((author) => {
      ...
      else res.status(404).send('No author found')
    })
    .catch((err) => {
  ...
  Author.findById(address)
    .then((foundAuthor) => {
      if (foundAuthor) {
        res
          .status(400)
          .json({ message: 'Author with address already registered' })
        throw new Error(
          'Author with address' + address + ' already registered!',
        )
      }
      return author.save()
    })
...
exports.updateAuthor = (req, res) => {
  ...
  let address = req.params.address
  res.status(200)
  return Author.findByIdAndUpdate(address, newAuthor, { new: true })
    .then((updatedAuthor) => {
      console.log('updatedAuthorupdatedAuthor', updatedAuthor)
...
exports.getAllAuthorNames = (_, res) => {
  Author.find({})
    ...
    .then((authors) => {
      ...
      if (authors)
        // check no authors case.
        res.status(200).json(authorsMap)
      // Return the map.
      else res.status(404).send('No authors found')
    })
    .catch((err) => {
      res.status(500).send(err)
    })
